...
class DetectorFadiga:
    def __init__(self):
        ...
        # Olho direito (lado esquerdo da imagem)
        self.p_olho_dir = [160, 144, 158, 153, 33, 133]
        # Olho esquerdo (lado direito da imagem)
        ...
        self.model_path = None  # MediaPipe irá usar o modelo padrão
        self.num_threads = 4    # Número de threads para processamento
    ...
    # Os landmarks são os pontos de referência do rosto
    def _suavizar_landmarks(self, current_landmarks):
        ...
        for i, landmark in enumerate(current_landmarks.landmark):
            ...
            smooth_x = prev.x * self.landmark_smoothing + \
                landmark.x * (1 - self.landmark_smoothing)
            smooth_y = prev.y * self.landmark_smoothing + \
                landmark.y * (1 - self.landmark_smoothing)
            smooth_z = prev.z * self.landmark_smoothing + \
                landmark.z * (1 - self.landmark_smoothing)
            landmark.x, landmark.y, landmark.z = smooth_x, smooth_y, smooth_z

        self.last_valid_landmarks = current_landmarks
        return current_landmarks

    def _desenhar_pontos_orientacao(self, frame, face_landmarks):
        """Desenha os pontos usados para verificar a orientação do rosto"""
        ...
        PONTOS = {
            ...
            'Queixo': 152,
            # Pontos da boca
            'Labio Sup': 13,    # Centro do lábio superior
            'Labio Inf': 14,    # Centro do lábio inferior
            'Boca Esq': 61,     # Canto esquerdo da boca
            'Boca Dir': 291,    # Canto direito da boca
            'Labio Sup Ext': 0,  # Arco do cupido esquerdo
            'Labio Inf Ext': 17,  # Parte mais baixa do lábio inferior

            # Pontos dos olhos (corrigidos)
            # Olho Esquerdo (lado direito da imagem)
            'Olho Esq Superior': 159,
            'Olho Esq Inferior': 145,
            'Olho Esq Interno': 133,
            'Olho Esq Externo': 33,
            'Olho Esq Sup Int': 160,
            'Olho Esq Sup Ext': 144,
            'Olho Esq Inf Int': 158,
            'Olho Esq Inf Ext': 153,

            # Olho Direito (lado esquerdo da imagem)
            'Olho Dir Superior': 386,
            'Olho Dir Inferior': 374,
            'Olho Dir Interno': 362,
            'Olho Dir Externo': 263,
            'Olho Dir Sup Int': 385,
            'Olho Dir Sup Ext': 380,
            'Olho Dir Inf Int': 387,
            'Olho Dir Inf Ext': 373,
        }

        # Cores para cada ponto
        CORES = {
            'Nariz (Ponta)': (0, 0, 255),    # Vermelho
            'Nariz (Base)': (0, 0, 255),     # Vermelho
            'Orelha Esq': (0, 255, 0),       # Verde
            'Orelha Dir': (0, 255, 0),       # Verde
            'Olho Esq': (255, 0, 0),         # Azul
            'Olho Dir': (255, 0, 0),         # Azul
            'Queixo': (255, 255, 0),         # Ciano

            # Cores para pontos da boca
            'Labio Sup': (255, 0, 255),      # Magenta
            'Labio Inf': (255, 0, 255),      # Magenta
            'Boca Esq': (165, 42, 42),       # Marrom
            'Boca Dir': (165, 42, 42),       # Marrom
            'Labio Sup Ext': (255, 182, 193),  # Rosa claro
            'Labio Inf Ext': (255, 182, 193),  # Rosa claro

            # Cores para pontos dos olhos
            'Olho Esq Superior': (255, 0, 0),     # Azul
            'Olho Esq Inferior': (255, 0, 0),     # Azul
            'Olho Esq Externo': (0, 255, 255),    # Amarelo
            'Olho Esq Interno': (0, 255, 255),    # Amarelo
            'Olho Esq Sup Int': (0, 165, 255),    # Laranja
            'Olho Esq Sup Ext': (0, 165, 255),    # Laranja
            'Olho Esq Inf Int': (128, 0, 255),    # Roxo
            'Olho Esq Inf Ext': (128, 0, 255),    # Roxo

            'Olho Dir Superior': (255, 0, 0),     # Azul
            'Olho Dir Inferior': (255, 0, 0),     # Azul
            'Olho Dir Externo': (0, 255, 255),    # Amarelo
            'Olho Dir Interno': (0, 255, 255),    # Amarelo
            'Olho Dir Sup Int': (0, 165, 255),    # Laranja
            'Olho Dir Sup Ext': (0, 165, 255),    # Laranja
            'Olho Dir Inf Int': (128, 0, 255),    # Roxo
            'Olho Dir Inf Ext': (128, 0, 255),    # Roxo
        }
        ...
        for nome, idx in PONTOS.items():
            ...
            # Desenhar círculo maior para pontos dos olhos
            raio = 3  # Menor para pontos dos olhos
            if 'Olho' in nome:
                raio = 3
            elif 'Labio' in nome or 'Boca' in nome:
                raio = 5

            # Desenhar círculo e texto com fonte menor para os olhos
            cv2.circle(frame, (pos_x, pos_y), raio, CORES[nome], -1)
            fonte_tamanho = 0.3 if 'Olho' in nome else 0.5
            cv2.putText(frame, nome, (pos_x + 5, pos_y - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, fonte_tamanho, CORES[nome], 1)
    ...
    def _detectar_piscada(self, ear):
        """Detecta piscadas"""
        EAR_PISCADA = 0.25      # Aumentado para 0.25 para maior sensibilidade
        DURACAO_MAX = 0.3       # Aumentado para 0.3s para melhor detecção
        COOLDOWN = 0.1          # Reduzido para 0.1s para detectar piscadas mais frequentes

        tempo_atual = time.time()

        # Se estiver no período de cooldown, não detecta nova piscada
        if tempo_atual - self.ultimo_tempo_piscada < COOLDOWN:
            return False

        # Detecta início da piscada
        if ear < EAR_PISCADA and not self.piscando:
            self.piscando = True
            self.ultimo_tempo_piscada = tempo_atual
            return False

        # Detecta fim da piscada
            ...
            if duracao < DURACAO_MAX:
                ...
                print(f"Piscada detectada! EAR: {ear:.2f}")  # Debug
            self.piscando = False
            return True
    ...
    def _verificar_qualidade_deteccao(self, landmarks):
        ...
        face_rotation = abs(
            landmarks[self.p_olho_esq[0]].y - landmarks[self.p_olho_dir[0]].y)
    ...
    def _desenhar_texto_centralizado(self, frame, texto, cor=(0, 0, 255), escala=1.2):
        ...
        cv2.putText(frame, texto, (pos_x, pos_y), fonte,
                    escala, (0, 0, 0), espessura + 1)
        cv2.putText(frame, texto, (pos_x, pos_y),
                    fonte, escala, cor, espessura)

    def _verificar_orientacao_rosto(self, landmarks):
        """Verifica se o rosto está muito rotacionado ou inclinado"""
        # Pontos para nariz, orelhas, olhos e queixo, esses pontos são usados para verificar a orientação do rosto
        NARIZ_PONTA = 4  # Ponto da ponta do nariz
        NARIZ_BASE = 168  # Ponto da base do nariz
        ORELHA_ESQ = 234
        ORELHA_DIR = 454
        OLHO_ESQ_EXTERNO = 33
        OLHO_DIR_EXTERNO = 263
        QUEIXO = 152

        # Converter landmarks para array numpy
        nariz_ponta = np.array(
            [landmarks[NARIZ_PONTA].x, landmarks[NARIZ_PONTA].z])
        nariz_base = np.array(
            [landmarks[NARIZ_BASE].x, landmarks[NARIZ_BASE].z])
        orelha_esq = np.array(
            [landmarks[ORELHA_ESQ].x, landmarks[ORELHA_ESQ].z])
        orelha_dir = np.array(
            [landmarks[ORELHA_DIR].x, landmarks[ORELHA_DIR].z])
        olho_esq = np.array([landmarks[OLHO_ESQ_EXTERNO].y])
        olho_dir = np.array([landmarks[OLHO_DIR_EXTERNO].y])
        queixo = np.array([landmarks[QUEIXO].y])

        # Calcular rotação lateral usando profundidade (coordenada z)
        prof_nariz = np.linalg.norm(nariz_ponta - nariz_base)
        dist_orelhas = np.linalg.norm(orelha_esq - orelha_dir)
        razao_profundidade = prof_nariz / dist_orelhas if dist_orelhas > 0 else 1

        # Calcular inclinação vertical
        dist_olhos = abs(olho_esq - olho_dir)
        dist_vertical = abs(np.mean([olho_esq, olho_dir]) - queixo)
        razao_vertical = dist_olhos / dist_vertical if dist_vertical > 0 else 0

        # Thresholds ajustados
        THRESHOLD_LATERAL = 0.4     # Reduzido para maior sensibilidade
        THRESHOLD_VERTICAL = 0.15   # Ajustado para inclinação vertical

        # Verificar rotação lateral
        if razao_profundidade > THRESHOLD_LATERAL:
            return False, "Rosto rotacionado lateralmente"

        # Verificar inclinação vertical
        if razao_vertical > THRESHOLD_VERTICAL:
            return False, "Rosto inclinado verticalmente"
                ...
                if resultados.multi_face_landmarks:
                    ...
                    if all(0.0 <= l.x <= 1.0 and 0.0 <= l.y <= 1.0 for l in landmarks):
                        if self._verificar_qualidade_deteccao(landmarks):
                            face_landmarks = self._suavizar_landmarks(
                            ...
                            # Adicionar esta linha para desenhar os pontos de orientação
                            self._desenhar_pontos_orientacao(
